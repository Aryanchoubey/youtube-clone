...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordValid) {
    throw new ApiError(404, "invalid use credential");
  }
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (user.avatar) {
    const oldAvatarPath = path.join(user.avatar); // stored path like "uploads/old.png"

    if (fs.existsSync(oldAvatarPath)) {
      fs.unlinkSync(oldAvatarPath); // delete file
    }
  }

  // 4. Save new avatar path in DB
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(200, " cover Image uploaded successfully");
});
